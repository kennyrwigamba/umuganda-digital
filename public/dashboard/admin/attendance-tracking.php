    ...
    if (! $selectedEvent) {
        ...
        try {
            ...
            SELECT u.id as user_id, u.first_name, u.last_name, u.email,
                   c.name as cell_name, a.id as attendance_id, a.status, a.check_in_time, a.notes, a.excuse_reason,
...
<?php if (! $record['notes'] && ! $record['excuse_reason']): ?>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                <?php if ($record['attendance_id']): ?>
                                                    <div class="flex space-x-2">
                                                        <!-- View Button -->
                                                        <button onclick="viewAttendance(<?php echo $record['attendance_id']; ?>)"
                                                        ...
                                                        <button onclick="editAttendance(<?php echo $record['attendance_id']; ?>)"
                                                                class="inline-flex items-center px-3 py-1 rounded-md text-xs font-medium bg-yellow-100 text-yellow-700 hover:bg-yellow-200 transition-colors"
                                                                title="Edit Record">
    ...
    <script>
        ...
        function viewAttendance(attendanceId) {
            // Create modal to show attendance details
            fetch(`attendance-details.php?id=${attendanceId}`)
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        showAttendanceModal(data.record, 'view');
                    } else {
        ...
        function editAttendance(attendanceId) {
            // Create modal to edit attendance details
            fetch(`attendance-details.php?id=${attendanceId}`)
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        showAttendanceModal(data.record, 'edit');
                    } else {
                        alert('Error loading attendance details: ' + data.message);
                    }
                })
        ...
        // Show attendance modal (view or edit mode)
        function showAttendanceModal(record, mode) {
            const isEditable = mode === 'edit';
            const modalTitle = isEditable ? 'Edit Attendance Record' : 'View Attendance Details';

            const modalHTML = `
                <div id="attendanceModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
                    <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-2/3 lg:w-1/2 shadow-lg rounded-md bg-white">
                        <div class="mt-3">
                            <h3 class="text-lg font-medium text-gray-900 mb-4">${modalTitle}</h3>
                            <form id="attendanceForm" ${isEditable ? 'onsubmit="saveAttendance(event)"' : ''}>
                                <input type="hidden" id="attendance_id" value="${record.id}">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p class="mt-1 text-sm text-gray-900">${record.event_title}</p>
                                    </div>

                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">Status</label>
                                        ${isEditable ? `
                                            <select id="status" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500">
                                        ...
                                        ` : `
                                            <span class="mt-1 inline-flex px-2 py-1 text-xs font-medium rounded-full ${getStatusBadgeClass(record.status)}">
                                    ...
                                    <div>
                                        ...
                                        ${isEditable ? `
                                            <input type="datetime-local" id="check_in_time" value="${record.check_in_time}"
                                                   class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500">
                                        ` : `
                                            <p class="mt-1 text-sm text-gray-900">${record.check_in_time || '-'}</p>
                                        `}
                                    </div>
                                    ...
                                    <div class="md:col-span-2">
                                        ...
                                        ${isEditable ? `
                                            <textarea id="notes" rows="3"
                                                      class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500"
                                                      placeholder="Add notes...">${record.notes || ''}</textarea>
                                        ` : `
                                    ...
                                    ${record.excuse_reason ? `
                                        <div class="md:col-span-2">
                                            <label class="block text-sm font-medium text-gray-700">Excuse Reason</label>
                                            ${isEditable ? `
                                                <textarea id="excuse_reason" rows="2"
                                                          class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500">${record.excuse_reason || ''}</textarea>
                                            ` : `
                                                <p class="mt-1 text-sm text-gray-900">${record.excuse_reason}</p>
                                            `}
                                        </div>
                                    ` : ''}
                                    ...
                                    <div>
                                        ...
                                        <p class="mt-1 text-sm text-gray-900">${record.recorded_by_name || '-'}</p>
                                    </div>

                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">Record Date</label>
                                        <p class="mt-1 text-sm text-gray-900">${record.created_at || '-'}</p>
                                    </div>
                                </div>
        ...
        function saveAttendance(event) {
            ...
            const formData = {
                id: document.getElementById('attendance_id').value,
                status: document.getElementById('status').value,
                check_in_time: document.getElementById('check_in_time').value,
                notes: document.getElementById('notes').value,
                excuse_reason: document.getElementById('excuse_reason') ? document.getElementById('excuse_reason').value : ''
            };

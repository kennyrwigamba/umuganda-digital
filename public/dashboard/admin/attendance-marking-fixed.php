            ...
            foreach ($attendanceData as $data) {
                ...
                if (isset($data['fine_amount']) && $data['fine_amount'] > 0) {
                    ...
                    if ($fineExists) {
                        ...
                        // Insert new fine
                        $insertFineQuery = "INSERT INTO fines (attendance_id, amount, reason, status, created_at)
                                       VALUES (?, ?, ?, 'pending', NOW())";
                        $insertFineStmt = $connection->prepare($insertFineQuery);
                        $reason         = $data['status'] === 'late' ? 'Late arrival' : 'Absence';
                        $insertFineStmt->bind_param('ids', $attendanceId, $data['fine_amount'], $reason);
...
<head>
    ...
    <title>Attendance Marking - Umuganda Tracker</title>
